...
ee1 <- merged_final %>% group_by(family_profile) %>% 
  ...
  transmute(`N вузов в выборке` = n, `% в выборке` = sh_our)

ee2 <- viborka %>% group_by(family_profile) %>% 
  count() %>% 
  mutate(sh_our = 100*n/502) %>% 
  transmute(`N вузов в Мониторинге` = n, `% в Мониторинге` = sh_our)
...
ee5 <- viborka %>% 
  ...
  transmute(family_profile = "Всего",
            `N вузов в Мониторинге` = n, 
            `% в Мониторинге` = sh_our)
...
#ee7$`N вузов в выборке`[10]="-"
#ee7$`% в выборке`[10]="-"
knitr::kable(ee7 , booktabs = T, digits = 0) %>%
 kable_styling() %>% 
row_spec(11, bold = TRUE) 
```

<span style="color: #c1c1d7"> _Примечание: Здесь и далее разбивка по семействам на основе: [Соколов 2017](https://vo.hse.ru/data/2017/06/28/1171155834/Sokolov.pdf)._<span>

Немного подробнее о самих отчетах: они имеют матричную структуру (образец упрощенной формы представлен в [Приложении 2](#ap2). В отчете указана не только общая сумма доходов по различным статьям, но и источник финансирования этих доходов. То есть мы можем видеть, что вуз оказал услуг на 100 рублей, при этом 60 рублей получил из бюджета, а 40 рублей из внебюджетных источников (фирмы и домохозяйства). То же самое и с расходами: мы видим, что вуз заплатил сотрудникам 100 рублей, и мы точно знаем, какая часть из этих 100 рублей — бюджетные деньги, а какая внебюджетные. Основным ограничением для нас является то, что доходы и расходы разбиты на укрупненные группы, поэтому мы не можем узнать о содержании, например, строки «Доходы от оказания платных услуг». Внутри будут находиться не только образовательная деятельность вуза, но и научная. Таким образом, известно, откуда пополнялась эта строка — из бюджета или от фирм/домохозяйств, но неясно, какая часть поступила от образовательных программ, а какая от научных исследований. 

Распределение доходов среднего вуза представлено в [табл. 2.](#ap2) Внебюджетные доходы составляют 37% от всех доходов вуза, причем основная часть внебюджетных доходов — это доходы от оказания платных услуг (90%), то есть деньги за образовательные услуги и за научные исследования. В остальные 10% входят доходы от собственности, возмещение ущерба, операции с активами и прочее.
...
t3 <- t3 %>% 
  ...
  gather(key=agencyId, value=Value, inc_sh_gov, inc_sh_mark) %>% 
  mutate(agencyId = case_when(
    agencyId == "inc_sh_gov" ~ "Доходы от бюджетов всех уровней",
    TRUE   ~  "Доходы от приносящей доход деятельности")) 
...
r1a <- merged_final %>% 
  ...
  select(short_name, code_040_130_availableMeansEndYear, budg, sh_mark_tot, tot, agencyId, family_profile) #%>% filter(sh_mark_tot<77)
...
knitr::kable(t_r4 , booktabs = T, digits = 0) %>%
 kable_styling() %>% 
...
Далее разобьем вузы по двум группам: столичные вузы (Москва и Санкт-Петербург) и региональные вузы (остальные регионы России). Повторим тот же график, как на рис. 2, но с учетом этой группировки. На [рис. 4](#ris4) мы видим, что облако столичных вузов в основном перемешано с облаком региональных, но всю верхнюю область этого облака занимают преимущественно черные точки, т. е. столичные вузы. Таким образом, когда мы говорим о вузах с самыми большими доходами от оказания платных услуг (в рублях), то это Москва и Санкт-Петербург, но когда мы смотрим на доли в общих доходах, то средний столичный вуз мало чем отличаются от среднего регионального по способности зарабатывать на платных услугах. 
...
```{r михаил1, eval = FALSE, fig.align = "center", out.width = '80%'}
...
r7_2 <- merged_final %>% 
  mutate(
    gov_cos_sh_wage = 100*(code_160_210_budgetActivityEndYear+code_160_210_incomeActivityEndYear)/(code_150_200_budgetActivityEndYear+code_150_200_incomeActivityEndYear),
    gov_cos_sh_rab_usl = 100*(code_170_220_budgetActivityEndYear + code_170_220_incomeActivityEndYear)/(code_150_200_budgetActivityEndYear+code_150_200_incomeActivityEndYear),
    gov_cos_sh_ostalnoe =100*(code_190_230_budgetActivityEndYear+code_190_230_incomeActivityEndYear+
                              code_210_240_budgetActivityEndYear+code_210_240_incomeActivityEndYear+
                              code_230_250_budgetActivityEndYear+code_230_250_incomeActivityEndYear+
                              code_240_260_budgetActivityEndYear+code_240_260_incomeActivityEndYear+
                              code_250_290_budgetActivityEndYear+code_250_290_incomeActivityEndYear+
                              code_260_270_budgetActivityEndYear+code_260_270_incomeActivityEndYear)/(code_150_200_budgetActivityEndYear+code_150_200_incomeActivityEndYear)) %>% 
  mutate(
    cos_sh_wage = 100*code_160_210_availableMeansEndYear/code_150_200_availableMeansEndYear,
    cos_sh_rab_usl = 100*code_170_220_availableMeansEndYear/code_150_200_availableMeansEndYear,
    cos_sh_ostalnoe = 100*(code_190_230_availableMeansEndYear+
                              code_210_240_availableMeansEndYear+
                              code_230_250_availableMeansEndYear+
                              code_240_260_availableMeansEndYear+
                              code_250_290_availableMeansEndYear+
                              code_260_270_availableMeansEndYear)/code_150_200_availableMeansEndYear) %>%
  select(2, cos_sh_wage, gov_cos_sh_wage, cos_sh_rab_usl,gov_cos_sh_rab_usl, cos_sh_ostalnoe, gov_cos_sh_ostalnoe) %>% filter(agencyId != 164694) %>% select(-1)
  
r7_2[r7_2$cos_sh_wage == "NaN", "cos_sh_wage"] <- 0
r7_2[r7_2$cos_sh_rab_usl == "NaN", "cos_sh_rab_usl"] <- 0
r7_2[r7_2$cos_sh_ostalnoe == "NaN", "cos_sh_ostalnoe"] <- 0

J <- basicStats(r7_2)
J <- data.frame(t(J))
J <- J %>% select(1,3,4,5,6,7,8) %>% rownames_to_column("name")

J[J$name == "cos_sh_wage", "name"] <- "Оплата труда, финансируемеая внебюджетными источниками"  
J[J$name == "gov_cos_sh_wage", "name"] <- "Оплата труда, финансируемеая из бюджетов всех уровней" 
J[J$name == "cos_sh_rab_usl", "name"] <- "Оптата работ(услуг), финансируемеая внебюджетными источниками"  
J[J$name == "gov_cos_sh_rab_usl", "name"] <- "Оптата работ(услуг), финансируемеая из бюджетов всех уровней" 
J[J$name == "cos_sh_ostalnoe", "name"] <- "Прочее, финансируемое внебюджетными источниками"  
J[J$name == "gov_cos_sh_ostalnoe", "name"] <- "Прочее, финансируемое из бюджетов всех уровней" 

J[J$nobs == 389, "nobs"] <- 390  
...
## Возможно ли перераспределить доходы так, чтобы не сокращать особо чувствительные статьи расходов?
...
colnames(t8)[1] <- "Вуз" 
colnames(t8)[3] <- "Остальные расходы из всех источников кроме оплаты труда (млн руб.)"
colnames(t8)[2] <- "Оплата труда из средств студентов и фирм (млн руб.)"
colnames(t8)[4] <- "Доля зарплат, которые все равно должны платиться из внебюджета при максимальном перераспределении средств бюджетов всех уровней на эти цели (%)"
colnames(t8)[5] <- "Расходы на оплату труда из всех источников (млн руб.)"
...
t8 %>% select(1,5,2,3,4) %>% 
  ...
  formatStyle("condition", target = 'row', 
   ...
   formatStyle(columns = c(1), fontSize = '80%') %>% 
  formatRound(columns=c(2:6), digits=0, interval = 0,
  mark = "") 
```

```{r михаил4, eval=FALSE}
df <- merged_final %>% 
  mutate(vneb_vse_ne_zp = 
           code_190_230_availableMeansEndYear+
           code_210_240_availableMeansEndYear+
           code_230_250_availableMeansEndYear+
           code_240_260_availableMeansEndYear+
           code_250_290_availableMeansEndYear+
           code_260_270_availableMeansEndYear+
           code_170_220_availableMeansEndYear,
         ostat = (code_150_200_totalEndYear-code_160_210_totalEndYear-vneb_vse_ne_zp)/1000000,
         ostat_old = (code_150_200_totalEndYear-code_160_210_totalEndYear)/1000000,
         all_inc = code_010_100_availableMeansEndYear/1000000, 
    mark_inc = code_040_130_availableMeansEndYear/1000000, 
    wage = code_160_210_availableMeansEndYear/1000000,
    dang = (code_160_210_availableMeansEndYear - (code_010_100_availableMeansEndYear - code_040_130_availableMeansEndYear))/1000000,
    ...
    all_cost = 100*code_160_210_totalEndYear/code_150_200_totalEndYear,
    tot_cost = code_150_200_totalEndYear/1000000,
    sh =  100*(dang - ostat)/tot_cost,
         sh2 =  dang - ostat, 
    sh3 = 100*ostat/dang) %>% 
  select(short_name, dang, ostat, ostat_old, sh, all_w, tot_cost, sh2,sh3) %>%
  mutate(short_name = gsub("\\ ИМЕНИ .*","",short_name),
         short_name = gsub("\\ ИМ. .*","",short_name),
         short_name = gsub("\\ МИНИСТЕРСТВА.*","",short_name),
         short_name = gsub("\\ ПРИ .*","",short_name),
...
colnames(t8)[4] <- "Покрытие оплаты труда, финансируемой студентами и фирмами [4]-[5] в % к [1]"
colnames(t8)[5] <- "Расходы на зарплату из всех источников (млн руб)"
colnames(t8)[6] <- "Все расходы из всех источников (млн руб)"

t8 %>% select(1,6,5,2,3,4,7) %>% 
  mutate(condition = (.[[2]] > 1)) %>% 
  datatable(rownames = FALSE, options = list(
    pageLength = 5,
...
```{r}
t1 <- t_base %>% 
  mutate(sh =  100*(dang - ostat)/all_w) %>% 
  select(short_name, dang, ostat, sh, all_w, tot_cost, family_profile) %>% filter(sh>0) %>% 
...
t4 <- t_base %>% 
  mutate(sh =  dang - ostat) %>% 
  select(short_name, dang, ostat, sh, all_w, tot_cost, family_profile) %>% filter(sh>0) %>% 
  group_by(family_profile) %>% summarise(money = sum(sh))
...
t4 <- bind_rows(t4,t4_2)

t5 <- t3 %>% left_join(t4, by= "family_profile") 

colnames(t3)[1] <- "Семейство" 
colnames(t3)[2] <- "Вузы с ограниченным маневром"
colnames(t3)[3] <- "Всего вузов в семействе"
colnames(t3)[4] <- "% вузов в семействе с ограниченным маневром"
colnames(t3)[5] <- "Сревдсва, которые выделяются на зарплаты из внебюджетных средств (млн. руб)"
...
1. Внебюджетные доходы составляют в среднем 37% от всех доходов вуза; только для 25% вузов в нашей выборке доля внебюджетных доходов превышает 47%. 

2. Бóльшая часть внебюджетных доходов — это доходы от оказания платных услуг по образовательным программам и проведению исследований (в среднем 90% внебюджетных средств). Причем исследования приносят значительно меньше средств в сравнении с доходами от образовательных программ. 

3. Вузы, которые оперируют бóльшими самостоятельно заработанными суммами в рублях, являются и более независимыми от бюджетных денег (имеют бóльшую долю доходов от платных услуг).

4. Внебюджетные средства распределены неравномерно по типам вузов. Больше всех по соотношению к бюджетным деньгам зарабатывают социально-экономические и медицинские вузы — примерно половина их доходов поступает от оказания платных услуг населению и фирмам. Хуже всего зарабатывают силовые, спортивные, культуры и искусств. Самые многочисленные семейства вузов — технические и классические вузы — имеют треть своих доходов из внебюджетных источников. 

5. Внутри семейств нет сильно заметного разброса значений по внебюджетным доходам (за исключением муниципальных вузов). Если в среднем социально-экономические вузы зарабатывают больше остальных вузов, то это свойственно почти всем вузам этого типа, так же и наоборот.

6. Вузы с самыми большими доходами от оказания услуг населению (в рублях) — это чаще столичные вузы; среди региональных вузов больше тех вузов, для которых внебюджетные деньги — главный доход.

7. Социально-экономические и медицинские вузы больше остальных типов вузов получают доходы от платных услуг, однако они и больше остальных вузов направляют их на оплату труда — в среднем не менее 35% для первых и 30% для вторых. Другие типы вузов в основном оплату труда производят за счет бюджетного финансирования, что наиболее характерно для вузов культуры и искусств, а также силовых и спортивных вузов. 

8. Всего в нашей выборке мы зафиксировали 14% вузов, которым будет затруднительно перераспределить расходы в случае необходимости восполнить потери от внебюджетных средств. Такие вузы встречаются среди всех категорий, однако чаще это социально-экономические и медицинские вузы. 
...
##### <span style="color: #000000"> Приложение 1.  Распределение доходов государственных вузов по источникам их получения и видам деятельности в 2019 году (в млн руб.) <span> {#ap1}
...
knitr::kable(tab1 , booktabs = T, digits = 2, interval = t) %>%
 kable_styling() %>% 
...
##### <span style="color: #000000">  Приложение 4.  Описательные статистики расходов государственных вузов  <span> {#ap4}

```{r, fig.align = "center", out.width = '80%'}
r7 <- merged_final %>% mutate(
    gov_sh_tot = 100*(code_150_200_budgetActivityEndYear+code_150_200_incomeActivityEndYear)/code_150_200_totalEndYear,
    mark_sh_tot = 100*code_150_200_availableMeansEndYear/code_150_200_totalEndYear)%>% 
  select(2,gov_sh_tot, mark_sh_tot) 

r7 <- r7 %>% 
  gather(key=agencyId, value=Value, gov_sh_tot, mark_sh_tot) %>% 
  mutate(agencyId = case_when(agencyId == "gov_sh_tot" ~ "Расходы, финансируемые бюджетами всех уровней",
                              TRUE   ~  "Расходы, финансируемые приносящей доход деятельностью"))  %>% group_by(agencyId) %>% 
  summarise(mean = mean(Value),
            median = median(Value), 
            var = var(Value),
            uni_count = length(Value), 
            Q1 = quantile(Value, probs=c(0.25)),
            #Q2 = quantile(inc_sh_mark, probs=c(0.5)),
            Q3 = quantile(Value, probs=c(0.75)),
            min = min(Value), max = max(Value)
...
colnames(r7)[1] <- " " 
colnames(r7)[2] <- "mean" 
colnames(r7)[3] <- "madian" 
colnames(r7)[4] <- "Var" 
colnames(r7)[5] <- "n"
r7 <- r7 %>% select(1,5,8,9,6,7,2,3)

knitr::kable(r7 , booktabs = T, digits = 1) %>%
 kable_styling() 
```

##### <span style="color: #000000">  Приложение 5.  Распределение расходов, финансируемых бюджетами всех уровней <span> {#ap5}
...
r7 <- merged_final %>% mutate(
    ...
    gov_cos_sh_ostalnoe =100*(code_190_230_budgetActivityEndYear+code_190_230_incomeActivityEndYear+
                              ...
                              code_250_290_budgetActivityEndYear+code_250_290_incomeActivityEndYear+
                            code_260_270_budgetActivityEndYear+code_260_270_incomeActivityEndYear)/(code_150_200_budgetActivityEndYear+code_150_200_incomeActivityEndYear))%>% 
  select(2,gov_cos_sh_wage, gov_cos_sh_rab_usl,gov_cos_sh_ostalnoe) %>% 
  mutate(inc = gov_cos_sh_wage+ gov_cos_sh_rab_usl+gov_cos_sh_ostalnoe) %>% 
  arrange(desc(gov_cos_sh_ostalnoe)) %>% dplyr::filter(agencyId != 164694)
...
r7 <- r7 %>% 
  #dplyr::filter(cos_sh_prochee > 0) %>% 
  select(agencyId,gov_cos_sh_wage, gov_cos_sh_rab_usl,gov_cos_sh_ostalnoe) %>% 
  gather(key=agencyId, value=Value, gov_cos_sh_wage, gov_cos_sh_rab_usl,gov_cos_sh_ostalnoe) %>% 
  mutate(agencyId = case_when(agencyId == "gov_cos_sh_wage" ~ "оплата труда",
                              agencyId == "gov_cos_sh_rab_usl" ~ "оплата работ (услуг)",
                              TRUE   ~  "прочее")) 
...
r7 %>% 
  ...
  scale_color_manual(values=c("#999999", "#dd4930", "#E69F00"))+
  scale_fill_manual(values=c("#999999", "#dd4930", "#E69F00"))+
  geom_text(aes(y=115, x=23,label='mean = 10%'), size=3.5, color = "#999999")+
    geom_text(aes(y=65, x=73,label='mean = 63%'), size=3.5, color = "#dd4930")+
      geom_text(aes(y=65, x=38,label='mean = 27%'), size=3.5, color = "#E69F00")+
   labs(
    x = "Расходы вузов (в %)",
    y = "Число вузов", 
    fill = NULL
...
```{r михаил, eval=FALSE}
r7 <- merged_final %>% mutate(
    gov_cos_sh_wage = 100*(code_160_210_budgetActivityEndYear+code_160_210_incomeActivityEndYear)/(code_150_200_totalEndYear),
    gov_cos_sh_rab_usl = 100*(code_170_220_budgetActivityEndYear + code_170_220_incomeActivityEndYear)/(code_150_200_totalEndYear),
    gov_cos_sh_ostalnoe =100*(code_190_230_budgetActivityEndYear+code_190_230_incomeActivityEndYear+
                              code_210_240_budgetActivityEndYear+code_210_240_incomeActivityEndYear+
                              code_230_250_budgetActivityEndYear+code_230_250_incomeActivityEndYear+
                              code_240_260_budgetActivityEndYear+code_240_260_incomeActivityEndYear+
                              code_250_290_budgetActivityEndYear+code_250_290_incomeActivityEndYear+
                            code_260_270_budgetActivityEndYear+code_260_270_incomeActivityEndYear)/(code_150_200_totalEndYear))%>% mutate(cos_sh_wage = 100*code_160_210_availableMeansEndYear/code_150_200_totalEndYear,
                      cos_sh_rab_usl = 100*code_170_220_availableMeansEndYear/code_150_200_totalEndYear,
                     cos_sh_ostalnoe =  
                       100*(code_190_230_availableMeansEndYear+
                              code_210_240_availableMeansEndYear+
                              code_230_250_availableMeansEndYear+
                              code_240_260_availableMeansEndYear+
                              code_250_290_availableMeansEndYear+
                              code_260_270_availableMeansEndYear)/code_150_200_totalEndYear) %>% 
  select(2,gov_cos_sh_wage, 
         gov_cos_sh_rab_usl,
         gov_cos_sh_ostalnoe,
         cos_sh_wage,cos_sh_rab_usl,cos_sh_ostalnoe) %>% dplyr::filter(agencyId != 164694)

r7 <- r7 %>% 
  select(agencyId,gov_cos_sh_wage, gov_cos_sh_rab_usl,gov_cos_sh_ostalnoe, cos_sh_wage, cos_sh_rab_usl,cos_sh_ostalnoe) %>% 
  gather(key=agencyId, value=Value, gov_cos_sh_wage, gov_cos_sh_rab_usl,gov_cos_sh_ostalnoe,cos_sh_wage, cos_sh_rab_usl,cos_sh_ostalnoe) %>% 
  mutate(agencyId = case_when(agencyId == "gov_cos_sh_wage" ~ "оплата труда (бюджет)",
                              agencyId == "gov_cos_sh_rab_usl" ~ "оплата работ, услуг (бюджет)",
                              agencyId == "gov_cos_sh_ostalnoe"   ~  "прочее (бюджет)",
                              agencyId == "cos_sh_wage" ~ "оплата труда (внебюджет)",
                              agencyId == "cos_sh_rab_usl" ~ "оплата работ, услуг (внебюджет)",
                              TRUE   ~  "прочее (внебюджет)")) 
r7 <- r7 %>%
group_by(agencyId) %>% 
  summarise(mean = mean(Value),
            median = median(Value), 
...
colnames(r7)[4] <- "Var" 
#colnames(r7)[5] <- "n"
r7 <- r7 %>% select(1,7,8,5,6,2,3)

knitr::kable(r7 , booktabs = T,  digits = c(0,0,0,0,0,1, 1)) %>%
 kable_styling() 
```
...
##### <span style="color: #000000"> Приложение 6. Пояснение к формулировке «как минимум»  <span> {#ap6}

> 
<font size="2"> 
Мы предполагаем, что вузы стремятся к сбалансированному бюджету — чтобы их доходы были равны расходам. Пусть у нас есть вуз со следующими данными в отчете:</font>
> 
<font size="2"> (1) Доходы от приносящей доход деятельности — 200 ед.:</font>
> 
<font size="2"> - оказание услуг студентам и фирмам — 160 ед.,</font>
> 
...
> 
<font size="2"> Мы видим, что всего доходов — 200 ед., а на оплату труда вуз тратит только 100 ед. Но точно неизвестно, что в этой сотне: возможно, все 100 ед. — это деньги студентов и организаций, а может быть, 60 от студентов, а 40 от собственности. Таким образом, из наших данных мы не сможем точно сказать, сколько зарплат было выплачено из денег студентов и фирм, но мы можем сказать о коридоре, в который попадает эта сумма. </font>
> 
<font size="2"> В нашем примере это коридор от 60 до 100 ед. То есть наш вуз финансирует деньгами студентов и фирм как минимум 60 ед. расходов на оплату труда и как максимум все 100 ед. Мы делаем оценку по нижней границе: 60 ед. точно идет на оплату труда (но возможно, и больше, вплоть до 100).</font>